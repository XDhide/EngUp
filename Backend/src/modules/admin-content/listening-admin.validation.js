...
function validateCreateLesson(req, res, next) {
  ...
  if (!body.transcript || typeof body.transcript !== 'string') errors.push('transcript là bắt buộc và phải là chuỗi');
...
function validateUpdateLesson(req, res, next) {
  const body = req.body || {};
  const allowedFields = ['title', 'transcript', 'difficulty', 'topic'];
...
module.exports = { validateCreateLesson, validateUpdateLesson, validateIdParam };
